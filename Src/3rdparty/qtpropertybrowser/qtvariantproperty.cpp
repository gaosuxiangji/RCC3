...
public:
    ...
    void slotEditingFinished(QtProperty *property);
...
void QtVariantPropertyManagerPrivate::slotEditingFinished(QtProperty * property)
{
	QtVariantProperty *varProp = m_internalToProperty.value(property, 0);
	if (!varProp)
		return;
	emit q_ptr->signalEditingFinished(varProp);
}
    ...
    : QtAbstractPropertyManager(parent), d_ptr(new QtVariantPropertyManagerPrivate)
{
    ...
    connect(intPropertyManager, SIGNAL(signalEditingFinished(QtProperty*)),
                this, SLOT(slotEditingFinished(QtProperty*)));
	...
	d_ptr->m_typeToPropertyManager[QVariant::UserType] = userType_FileNameManager;
	d_ptr->m_typeToValueType[QVariant::UserType] = QVariant::String;
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
	...
	else if (QtUserType_FileNameManager *FileNameManager = qobject_cast<QtUserType_FileNameManager *>(manager)) {
		return FileNameManager->value(internProp);
	}
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
		...
		FileNameManager->setValue(internProp, qvariant_cast<QString>(val));
		return;
	}
	else if (QtEnumPropertyManager *enumManager = qobject_cast<QtEnumPropertyManager *>(manager)) {
...
public:
	...
	QtUserType_FileNameFactory *m_FileNameFactory;
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent), d_ptr(new QtVariantEditorFactoryPrivate())
{
	...
	d_ptr->m_FileNameFactory = new QtUserType_FileNameFactory(this);
	d_ptr->m_factoryToType[d_ptr->m_FileNameFactory] = QVariant::UserType;
	d_ptr->m_typeToFactory[QVariant::UserType] = d_ptr->m_FileNameFactory;
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
	...
	QList<QtUserType_FileNameManager *> FileNameManagers = manager->findChildren<QtUserType_FileNameManager *>();
	QListIterator<QtUserType_FileNameManager *> itFileName(FileNameManagers);
	while (itFileName.hasNext()) {
		QtUserType_FileNameManager *manager = itFileName.next();
		d_ptr->m_FileNameFactory->addPropertyManager(manager);
	}
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
	...
	while (itColor.hasNext()) {
		QtUserType_FileNameManager *manager = itFileName.next();
		d_ptr->m_FileNameFactory->removePropertyManager(manager);
	}
