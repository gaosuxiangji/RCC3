...
class QtSpinBoxFactoryPrivate : public EditorFactoryPrivate<QFixSpinBox>
{
...
void QtSpinBoxFactoryPrivate::slotPropertyChanged(QtProperty *property, int value)
{
    if (!m_createdEditors.contains(property))
        return;
    QListIterator<QFixSpinBox *> itEditor(m_createdEditors[property]);
    while (itEditor.hasNext()) {
		QFixSpinBox *editor = itEditor.next();
...
void QtSpinBoxFactoryPrivate::slotSetValue(int value)
{
    ...
    const QMap<QFixSpinBox *, QtProperty *>::ConstIterator  ecend = m_editorToProperty.constEnd();
    for (QMap<QFixSpinBox *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor !=  ecend; ++itEditor) {
        if (itEditor.key() == object) {
            QtProperty *property = itEditor.value();
            QtIntPropertyManager *manager = q_ptr->propertyManager(property);
            if (!manager)
...
void QtSpinBoxFactoryPrivate::slotEditingFinished()
{
	...
	for (QMap<QFixSpinBox *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor) {
		if (itEditor.key() == object) {
			...
			if (!manager)
				return;
			manager->slotEditingFinished(property);
			return;
		}
	}
}

QFixSpinBox::QFixSpinBox(QWidget *parent /*= Q_NULLPTR*/)
	:QSpinBox(parent)
{
	setCorrectionMode(QAbstractSpinBox::CorrectToNearestValue);//自动修正时修正到邻近值
}

QFixSpinBox::~QFixSpinBox()
{

}

QValidator::State QFixSpinBox::validate(QString &s, int &i) const
{
...
void QFixSpinBox::fixup(QString &input) const
{
	//判断是否是数字,判断最大最小值
	bool isok = false;
	auto param = input.toDouble(&isok);
	if (isok)
	{
		if (param > maximum())
		{
		...
		input = QString().setNum((int)param);
	}
...
void QtSpinBoxFactory::connectPropertyManager(QtIntPropertyManager *manager)
{
	...
	connect(manager, SIGNAL(signalEditingFinished(QtProperty*)),
...
QWidget *QtSpinBoxFactory::createEditor(QtIntPropertyManager *manager, QtProperty *property,
        QWidget *parent)
{
	QFixSpinBox *editor = d_ptr->createEditor(property, parent);
...
void QtSpinBoxFactory::disconnectPropertyManager(QtIntPropertyManager *manager)
{
    ...
    disconnect(manager, SIGNAL(singleStepChanged(QtProperty*,int)),
                this, SLOT(slotSingleStepChanged(QtProperty*,int)));
	disconnect(manager, SIGNAL(editingFinished()), this, SLOT(slotEditingFinished()));
}
// QtSliderFactory
...
Q_SIGNALS:
	...
	void buttonClicked();
private:
	const QString m_const_btn_sheetStyle = "QToolButton{background-color:transparent;color:blue;border:none;}";
	QLabel *m_label;
	QToolButton *m_button;
	QString filePath;
};
...
{
	m_button->setStyleSheet(m_const_btn_sheetStyle);
...
{
	if (QString::number(0) != filePath) {
		setValue(QString::number(0));
