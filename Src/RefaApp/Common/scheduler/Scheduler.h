    ...
    public:
        explicit Scheduler(unsigned int max_n_tasks = 4) : done(false), threads(max_n_tasks + 1) {
          threads.push([this](int) {
			  Clock::time_point time_of_first_task;
              while (!done) {
				  {
				  ...
				  if (time_of_first_task.time_since_epoch() == Clock::duration::zero())
				  {
					  sleeper.sleep();
				  }
				  else
...
//    * * * * *
		...
		template<typename _Callable, typename... _Args>
		void delay_interval(const Clock::duration delay, const Clock::duration time, _Callable &&f, _Args &&... args) {
			std::shared_ptr<Task> t = std::make_shared<EveryTask>(time, std::bind(std::forward<_Callable>(f),
				std::forward<_Args>(args)...), true);
			add_task(Clock::now() + delay, std::move(t));
		}
