...
CSMeasureRadiusItem::CSMeasureRadiusItem(QGraphicsItem* parent, CMeasureLineManage::TMeasureLineInfo info)
...
void CSMeasureRadiusItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	Q_UNUSED(option);
	Q_UNUSED(widget);
	...
	if (m_modetype == CMeasureLineManage::MMT_Show || m_modetype == CMeasureLineManage::MMT_Add || m_modetype == CMeasureLineManage::MMT_Modify) {
		...
		if (m_item.vctPoint.size() >= 3)
		{
			...
			if (dbRadius > 0)
			{
				...
				vctPolyPoints.append(ptCenter);
				vctPolyPoints.append(ptVerter);
...
void CSMeasureRadiusItem::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
	m_nSelected_Index = -1;
	if (m_modetype == CMeasureLineManage::MMT_Modify) {
...
void CSMeasureRadiusItem::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
	if (m_modetype == CMeasureLineManage::MMT_Modify)
	{
...
void CSMeasureRadiusItem::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
	if (m_modetype == CMeasureLineManage::MMT_Modify)
	{
		if (m_nSelected_Index >= 0)
...
QRectF CSMeasureRadiusItem::boundingRect() const
{
	QRectF rc = QGraphicsLineItem::boundingRect();
	return rc;
}

QRectF CSMeasureRadiusItem::getFeaturePointRect(const QPointF& featurePt)
{
	return QRectF(featurePt.x() - Width / 2.0, featurePt.y() - Height / 2.0, Width, Height);
}

QPainterPath CSMeasureRadiusItem::shape() const
{
	QPainterPath prect;
