...
CSMeasureLineItem::CSMeasureLineItem(QGraphicsItem* parent, CMeasureLineManage::TMeasureLineInfo info)
...
void CSMeasureLineItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	Q_UNUSED(option);
	Q_UNUSED(widget);
	...
	if (m_modetype == CMeasureLineManage::MMT_Show || m_modetype == CMeasureLineManage::MMT_Add || m_modetype == CMeasureLineManage::MMT_Modify) {
		...
		strStaticTextValue = QString("%1").arg(QString::number(m_item.qrLength, 'f', 3));
...
void CSMeasureLineItem::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
	m_nSelected_Index = -1;
	if (m_modetype == CMeasureLineManage::MMT_Modify) {
...
void CSMeasureLineItem::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
	if (m_modetype == CMeasureLineManage::MMT_Modify)
	{
...
void CSMeasureLineItem::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
	if (m_modetype == CMeasureLineManage::MMT_Modify)
	{
		if (m_nSelected_Index >= 0)
...
QRectF CSMeasureLineItem::boundingRect() const
{
	QRectF rc = QGraphicsLineItem::boundingRect();
	return rc;
}

QPainterPath CSMeasureLineItem::shape() const
{
	QPainterPath prect;
...
QRectF CSMeasureLineItem::getFeaturePointRect(const QPointF& featurePt)
{
