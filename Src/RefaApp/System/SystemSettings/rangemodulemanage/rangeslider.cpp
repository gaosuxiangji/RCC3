    ...
    if(aEvent->buttons() & Qt::LeftButton)
    {
           ...
           && posCheck <= posMax - 2)
        {
			...
			if (posValue < firstHandleRectPosValue) {
				...
				emit lowerChandedFinished(mLowerValue);
			}
                    ...
                    && ((posValue < secondHandleRectPosValue) || !type.testFlag(RightHandle)))
            {
                if(type.testFlag(DoubleHandles))
					...
					else
					{
						...
						emit upperChandedFinished(mUpperValue);
					}
...
void RangeSlider::mouseReleaseEvent(QMouseEvent* aEvent)
{
    ...
    mFirstHandlePressed = false;
    mSecondHandlePressed = false;
	if (aHandleStateChangeIndex == 1) {
		setLowerValue(mLowerValue);
		emit lowerChandedFinished(mLowerValue);
	}
	else if (aHandleStateChangeIndex == 2)
	{
...
void RangeSlider::SetLowerValue(int aLowerValue, bool emitSig)
{
    setLowerValue(aLowerValue,emitSig);
}
...
void RangeSlider::SetUpperValue(int aUpperValue, bool emitSig)
{
    setUpperValue(aUpperValue,emitSig);
}
